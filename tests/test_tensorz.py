...
def test_mul_backward():
    ...
    assert np.allclose(z_a.grad, a.grad.numpy())
    assert np.allclose(z_b.grad, b.grad.numpy())
...
def test_pow_fuzzy():
    for _ in range(NUM_FUZZY):
        ...
        p = np.random.randint(low=-5, high=5, size=1)
...
def test_chain_ops():
    ...
    assert np.allclose(z_out.data, t_out.numpy())
    assert np.allclose(z_a.grad, t_a.grad.numpy())
    assert np.allclose(z_b.grad, t_b.grad.numpy())
